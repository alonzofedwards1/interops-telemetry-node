...
def _calc_duration_ms(req_ts: Optional[datetime], resp_ts: Optional[datetime]) -> int:
    if not req_ts or not resp_ts:
        return 0
    ms = int((resp_ts - req_ts).total_seconds() * 1000)
    return max(ms, 0)
...
def _extract_host_port(request_url: str) -> tuple[Optional[str], Optional[int]]:
    try:
...
def ingest_openhim_transactions() -> None:
    ...
    full_url = url + "?" + urlencode({"limit": limit})
    ...
    cert_cache: dict[tuple[str, int], Any] = {}
